...
if __name__ == "__main__":
    ...
    corpus = pd.read_parquet("path/to/corpus.parquet")
    corpus_text = "\n".join(corpus['contents'].tolist())

    # 执行评估并直接过滤
    loop = asyncio.get_event_loop()
    filtered_df = loop.run_until_complete(
        evaluator.evaluate(qa_df, corpus_text, score_threshold=80)
    )

    # 生成报告（基于过滤后的数据）
    evaluator.generate_report(filtered_df)

    # 保存结果（可选是否二次过滤）
    evaluator.save_report(filtered_df, "path/to/filtered_output.parquet")